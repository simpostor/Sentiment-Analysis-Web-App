            ...
            if file.filename.endswith('.csv'):
                try:
                    ...
                    if 'review' in df.columns:
                        ...
                        os.close(temp_fd)  # Close the file descriptor
                        plt.savefig(temp_chart_path, format='png', bbox_inches='tight')
                        plt.close()

                        # Set the global chart path
                        global_chart_path = temp_chart_path
...
def show_chart():
    ...
    if global_chart_path:
        return render_template("show_chart.html", global_chart_path=global_chart_path, global_csv_path=global_csv_path)
...
def show_chart_img():
    ...
    if global_chart_path:
        ...
        def cleanup(response):
            try:
                os.remove(global_chart_path)
            except Exception as e:
                print(f"Error removing temp file: {e}")
            return response

        return send_file(global_chart_path, mimetype='image/png')
